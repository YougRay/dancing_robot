...
rospy.init_node('dancing_pub')
pub1 = rospy.Publisher('/arm/joint1_position_controller',Float64,queue_size=10)
pub2 = rospy.Publisher('/arm/joint2_position_controller',Float64,queue_size=10)
pub5 = rospy.Publisher('/arm/joint5_position_controller',Float64,queue_size=10)
   
sys.setcheckinterval(1) #set switch interval to be 1/100th of a second by default
#initialize the audio capture device
inp = alsaaudio.PCM(type=alsaaudio.PCM_CAPTURE,mode=alsaaudio.PCM_NONBLOCK,device='hw:CARD=ArrayUAC10,DEV=2')
...
def danceFromArray(dArr, space = 2, iter = 1): #of shape n, 2, 3 and has values 0,1,2 for low middle high servos
    ...
    for _ in range(iter):
        for n in range(len(dArr)):
            ...
            for j in range(2): #for all servos
                for i in range(3):
                    if dArr[n][j][i] <= 2 and dArr[n][j][i] >1: #if we are moving between limits, take a weighted average
                        val[j][i] = limits[j][i][2]*(dArr[n][j][i]-1) + limits[j][i][1]*(2-dArr[n][j][i])
                    else:
                        val[j][i]= limits[j][i][1]*(dArr[n][j][i]) + limits[j][i][0]*(1-dArr[n][j][i])
                    #setPos(servoList[j][i],val) #set the servo position
            #ros
            pub1.publish(Float64(val[0][0]))
            pub2.publish(Float64(val[0][1]))  
            pub5.publish(Float64(val[0][2]))
            ...
            pub1.publish(Float64(0))
            pub2.publish(Float64(0))
            pub5.publish(Float64(0))
